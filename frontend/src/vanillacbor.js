...
(function () {
  ...
  const enforceUint8Array = (buffer) => {
    ...
    else return new Uint8Array(buffer.buffer.slice());
  ...
  const getTLVForNext = (buffer) => {
    ...
    let lennum =
      buffer[0] - (buffer[0] & 32) - (buffer[0] & 64) - (buffer[0] & 128);
    ...
    if (lennum < 24) {
      ...
      if (lennum === 24) {
        ...
        throw new Error(
          `Found a length of ${lennum}. Length values 28-30(0x1C-0x1E) are reserved!`
        );
      }
      ...
      switch (tlv.TAG) {
        case "UNSIGNED_INT":
          if (tlv.TLLEN === 1) result = tlv.VAL;
            ...
            throw new Error(
              "Ints over 4 bytes are not supported at this moment!"
            );
        ...
        case "NEGATIVE_INT":
          if (tlv.TLLEN === 1) result = -(1 + tlv.VAL);
            ...
            throw new Error(
              "Negative ints over 4 bytes are not supported at this moment!"
            );
        ...
        case "MAP":
          ...
          if (
            result.length !== tlv.VAL * 2 &&
            !(tlv.LEN === Infinity && result.length % 2 === 0)
          )
